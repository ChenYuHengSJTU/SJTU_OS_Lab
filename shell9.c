...
// if port use the argc and aggravate
    ...
    // int _len = sizeof(client_addr);
    // client_sockfd = accept(sockfd, (struct sockaddr *) &client_addr, &_len);
    ...
    while(1){
        ...
        // len = read(client_sockfd, dst, MAX_SIZE);
        if(len == 0){
            exit(0);
        }
